...
class CustomNavigation extends Component {
  ...
  render() {
    ...
    if (title) {
      ...
      nav_bar__center = (
        <View
          className="zdyl-nav-bar-search"
          style={`height:${capsulePosition.height}px;`}
          onClick={this.handleSearchClick.bind(this)}
        >
          <View className="zdyl-nav-bar-search__icon" />
          <View className="zdyl-nav-bar-search__input" style={{'color':background}}>{searchText}</View>
        </View>
      );
    } else {
      /* eslint-disable */
    ...
    return (
      <View
        className={`zdyl-nav-bar ${ios ? "ios" : "android"} ${extClass}`}
        style={`background: ${
          backgroundColorTop ? backgroundColorTop : background
        };height:${navBarHeight + navBarExtendHeight}px;`}
      >
        <View
          className={`zdyl-nav-bar__placeholder ${ios ? "ios" : "android"}`}
          style={`padding-top: ${navBarHeight + navBarExtendHeight}px;`}
        />
        <View
          className={`zdyl-nav-bar__inner ${ios ? "ios" : "android"}`}
          style={`background:${background};${navigationbarinnerStyle};`}
        >
          <View className="zdyl-nav-bar__left" style={navBarLeft}>
            {back && !home && (
              <View
                onClick={this.handleBackClick.bind(this)}
                className={`zdyl-nav-bar__button zdyl-nav-bar__btn_goback ${iconTheme}`}
              >
                <Text className="icon-global2 zdyl-nav-bar__btn_goback__icon-xiangyou" />
              </View>
            )}
            {!back && home && (
              <View
                onClick={this.handleGoHomeClick.bind(this)}
                className={`zdyl-nav-bar__button zdyl-nav-bar__btn_gohome ${iconTheme}`}
              />
            )}
            {back && home && (
              <View
                className={`zdyl-nav-bar__buttons ${ios ? "ios" : "android"}`}
              >
                <View
                  onClick={this.handleBackClick.bind(this)}
                  className={`zdyl-nav-bar__button zdyl-nav-bar__btn_goback ${iconTheme}`}
          ...
          <View
            className="zdyl-nav-bar__center"
            style={`padding-left: ${rightDistance}px`}
          >
            {nav_bar__center}
          </View>
          <View
            className="zdyl-nav-bar__right"
